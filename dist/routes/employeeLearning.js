...
const adminEmployeeLearningRouter = Router();
const portalEmployeeLearningRouter = Router();
...
function formatEmployeeContentRow(row, progress = null, completionCount = 0) {
  ...
  const category = row.category_label || CATEGORY_BY_TYPE[row.content_type] || "Document Verification";
...
async function getEmployeeLearningFeed(pool, employeeUserId) {
...
function sendEmployeeLearningFile(res, record, next) {
  streamStoredUpload(record.file_url || record.file_path, [record.file_name]).then((opened) => {
    if (opened?.stream) {
