...
const VideoSchema = z.object({
  ...
  youtubeUrl: z.string().min(1).transform((value) => normalizeYoutubeWatchUrl(value) || value).refine((value) => Boolean(normalizeYoutubeWatchUrl(value)), {
...
cmsRouter.post("/otp-settings/test", async (req, res, next) => {
  try {
    ...
    if (channel === "email") {
      ...
      const result2 = await sendOtpNotification({
        email: body.email,
        otp,
        channel: "email",
  ...
  try {
    ...
    await getPool().execute(
       ...
       WHERE id = :id`,
      {
        ...
        imageAlt: input.imageAlt ?? null,
        category: input.category ?? null,
        pub: input.isPublished === void 0 ? null : input.isPublished ? true : false,
