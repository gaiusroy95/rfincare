...
portalCustomerRouter.get("/financial-snapshot", authenticate, async (req, res, next) => {
  try {
    ...
    if (email && insurancePurchases.length === 0) {
      try {
        ...
        insurancePurchases = (orders || []).map((row) => ({
          ...
          status: row.payment_status === "paid" || row.insurer_push_status === "pushed" ? "active" : row.payment_status || "pending",
    ...
    const activeLoans = applications.filter(
      (a) => ["submitted", "under_review", "documents_pending", "approved", "disbursed"].includes(a.status)
    );
    const insurancePolicies = insurancePurchases.filter(
      (p) => ["active", "paid", "issued", "completed"].includes(String(p.status || "").toLowerCase())
    );
    ...
    const fixedDepositLeads = marketingLeads.filter((l) => {
      const data = safeJson(l.eligibility_data);
      return ["post_office", "fixed_income"].includes(l.loan_type) || ["post_office", "fixed_income"].includes(data?.marketplaceType);
    });
    const creditCardLeads = marketingLeads.filter(
      (l) => l.loan_type === "credit_card" || safeJson(l.eligibility_data)?.marketplaceType === "credit_card"
    );
    const pendingDocs = documents.filter(
      (d) => ["pending", "uploaded"].includes(String(d.verification_status || d.status || "pending").toLowerCase())
    );
    ...
    const verifiedDocs = documents.filter(
      (d) => ["verified", "approved"].includes(String(d.verification_status || d.status || "").toLowerCase())
    ).length;
    ...
    const recommendations = healthModel.improvementActions.filter((a) => a.pointsGain > 0).map((a) => ({
      type: a.category,
      title: a.title,
    ...
    const renewalAlerts = insurancePurchases.filter((p) => ["active", "issued", "paid"].includes(String(p.status || "").toLowerCase())).map((p) => ({
      id: p.id,
    ...
    const monthlySavings = mfSipOrders.filter((o) => ["mandate_pending", "active"].includes(o.status)).reduce((sum, o) => sum + (Number(o.sip_amount) || 0), 0);
    ...
    const financialGoals = savedGoals.length > 0 ? savedGoals : [];
    const upcomingPayments = [
      ...mfSipOrders.filter((o) => ["mandate_pending", "active"].includes(o.status)).slice(0, 2).map((o) => ({
        id: `sip-${o.id}`,
    ...
    res.json({
      ...
      sipPortfolio: [
        ...mfSipOrders.map((o) => ({
          ...
          resumePath: o.public_token ? `/mutual-fund-marketplace?sipId=${o.id}&sipToken=${o.public_token}` : null,
      ...
      claimStatus: insurancePurchases.filter((p) => String(p.status || "").toLowerCase().includes("claim")).map((p) => ({ id: p.id, product: p.product_name, status: p.status })),
      renewalAlerts,
      recommendations,
      financialHealthScore,
      healthGrade: healthModel.grade,
