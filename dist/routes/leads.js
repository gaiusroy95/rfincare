...
leadsRouter.get("/", authenticate, async (req, res, next) => {
  try {
    ...
    if (monthFilter) {
      ...
      const monthStart = /* @__PURE__ */ new Date(`${monthFilter}-01T00:00:00.000Z`);
    ...
    if (dateFrom) {
      const parsed = /* @__PURE__ */ new Date(`${dateFrom}T00:00:00.000Z`);
    ...
    if (dateTo) {
      const parsed = /* @__PURE__ */ new Date(`${dateTo}T00:00:00.000Z`);
      if (Number.isNaN(parsed.getTime())) {
