...
async function sendAppointmentEmail(opts) {
  ...
  if (opts.attachments?.length) {
    ...
    result = await sendEmail({ ...opts, attachments: void 0 });
  }
...
async function notifyAppointmentStaff({
  ...
  try {
    ...
    const emailTargets = /* @__PURE__ */ new Set();
    ...
    const primaryMail = await sendAppointmentEmail({
      ...
      attachments: icsAttachment ? [icsAttachment] : void 0,
      recipientName: "Rfincare Sales"
    });
    outcome.emailSent = Boolean(primaryMail?.sent);
    outcome.recipients = primaryMail?.sent ? [primaryTo, ...primaryMail?.channel === "smtp" ? bccList : []] : [];
...
appointmentsRouter.post("/", async (req, res, next) => {
  try {
    ...
    const icsAttached = customerEmailSent && Number(customerMail?.attachmentCount || 0) > 0 && !customerMail?.attachmentsDropped;
    ...
    res.status(201).json({
      ...
      notificationWarnings: {
        emailWarnings: [
          ...
          salesEmailSent ? null : "Sales team notification email could not be delivered right now. Your booking is still confirmed."
        ].filter(Boolean),
