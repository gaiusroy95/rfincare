...
function signSession() {
  ...
  const sig = crypto.createHmac("sha256", process.env.JWT_ACCESS_SECRET || "dev-panel-fallback").update(payload).digest("hex");
...
function verifySessionCookie(cookieValue) {
  ...
  const expectedSig = crypto.createHmac("sha256", process.env.JWT_ACCESS_SECRET || "dev-panel-fallback").update(payloadStr).digest("hex");
...
developmentRouter.put("/env", requireDevSession, async (req, res, next) => {
  try {
    ...
    if (input.target === "frontend") {
      ...
      saved.push("frontend");
      frontendMessage = frontendTarget.storageMode === "server-store" ? frontendTarget.hint : "Frontend .env saved. Reload the page or restart the Vite dev server to apply VITE_* values.";
    }
    res.json({
      success: true,
      saved,
      message: frontendMessage || "Environment files updated. Backend variables are active immediately."
    });
  } catch (err) {
    next(err);
  }
