...
const creditCardSavingsRouter = Router();
...
function mapDbCard(row) {
  ...
  const benefits = String(row.benefits || "").split("\n").map((s) => s.trim()).filter(Boolean);
  return {
    id: row.id,
    cardId: row.id,
    name: row.name,
    ...
    annualFeeWaiverSpendThreshold: row.annual_fee_waiver_spend_threshold != null ? Number(row.annual_fee_waiver_spend_threshold) : null,
...
creditCardSavingsRouter.post("/saver-recommendation", async (req, res, next) => {
  try {
    ...
    const defaultPeriod = input.spendPeriod || "monthly";
    const spends = normalizeSpends(input.spends, defaultPeriod);
    const cards = Array.isArray(input.cards) && input.cards.length ? input.cards : await loadActiveCards();
    if (!cards.length) {
