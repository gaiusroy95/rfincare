...
const BANK_LIST_CACHE_TTL_MS = 12e4;
...
const emptyToNull = (value) => value === "" || value === void 0 ? null : value;
...
const bankLogoUpload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (isBankLogoImage(file)) return cb(null, true);
...
function normalizeBankBody(body = {}, { partial = false } = {}) {
  const raw = { ...body };
  if (raw.logoUrl !== void 0 && raw.logo_url === void 0) raw.logo_url = raw.logoUrl;
  if (raw.logoAlt !== void 0 && raw.logo_alt === void 0) raw.logo_alt = raw.logoAlt;
  if (raw.bankType !== void 0 && raw.bank_type === void 0) raw.bank_type = raw.bankType;
  if (raw.displayPriority !== void 0 && raw.display_priority === void 0) {
    raw.display_priority = raw.displayPriority;
  }
  if (raw.reviewsCount !== void 0 && raw.reviews_count === void 0) {
    raw.reviews_count = raw.reviewsCount;
  }
  if (raw.customersServed !== void 0 && raw.customers_served === void 0) {
    raw.customers_served = raw.customersServed;
  }
  if (raw.partnershipDuration !== void 0 && raw.partnership_duration === void 0) {
    raw.partnership_duration = raw.partnershipDuration;
  }
  if (raw.applyUrl !== void 0 && raw.apply_url === void 0) raw.apply_url = raw.applyUrl;
...
function formatBankRow(row) {
  ...
  return {
    ...
    reviews_count: row.reviews_count != null && row.reviews_count !== "" ? Number(row.reviews_count) : 0,
    display_priority: row.display_priority != null && row.display_priority !== "" ? Number(row.display_priority) : 0,
...
function slimProductData(rawData) {
  ...
  return {
    ...
    product_category_slug: d.product_category_slug || d.productCategorySlug || d.catalog_slug || d.catalogSlug || null,
    ...
    foreclosure_allowed_after_months: d.foreclosure_allowed_after_months ?? d.foreclosureAllowedAfterMonths ?? null,
...
async function fetchBankList({ includeInactive, includeProducts, categoryQuery }) {
  ...
  const pool = getPool();
  const category = categoryQuery ? await resolveProductCategoryQuery(pool, categoryQuery) : null;
  ...
  const productsByBank = /* @__PURE__ */ new Map();
  for (const product of products) {
...
banksRouter.post(
  ...
  authenticate,
  authorize({ resource: "banks", action: "update" }),
  wrapMulter(bankLogoUpload.single("logo")),
  async (req, res, next) => {
    try {
      await ensureBankSchema();
      if (!req.file) {
      ...
      await pool.execute(
        `UPDATE banks SET logo_url = :logo_url WHERE id = :id`,
    ...
    try {
      ...
      await pool.execute(
         ...
         WHERE id = :id`,
        {
          ...
          customers_served: input.customers_served ?? null,
          partnership_duration: input.partnership_duration ?? null,
          certifications: input.certifications != null ? serializeCertifications(input.certifications) : null,
          apply_url: input.apply_url ?? null,
...
banksRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (rest.data != null) {
        productData = typeof rest.data === "string" ? (() => {
          try {
