...
const adminAgentLearningRouter = Router();
const portalAgentLearningRouter = Router();
const CONTENT_TYPES = /* @__PURE__ */ new Set([
  "video",
  "pdf",
  "presentation",
  "circular",
...
function formatContentRow(row, progress = null) {
  ...
  const thumbnailUrl = row.thumbnail_url || (String(row.mime_type || "").startsWith("image/") ? row.file_url : null);
  return {
    id: row.id,
    contentType: row.content_type,
    ...
    try {
      ...
      await pool.execute(
          ...
          :video_url, :file_size_bytes, :thumbnail_url, :is_new, :sort_order, :uploaded_by)`,
        {
          ...
          audience: req.body?.audience === "all" || contentType === "marketing" || contentType === "image" ? "all" : "agent",
          title,
          description: req.body?.description?.trim() || null,
          category_label: req.body?.categoryLabel?.trim() || req.body?.category_label?.trim() || (contentType === "marketing" || contentType === "image" ? "Investment" : null),
  ...
  try {
    ...
    await pool.execute(
         ...
         updated_at = NOW()`,
      {
        ...
        progress,
        completed_at: progress >= 100 ? /* @__PURE__ */ new Date() : null
      }
    );
...
async function getAgentLearningFeed(pool, agentUserId) {
