...
function getPublicApiBaseUrl() {
  return (process.env.API_PUBLIC_URL || process.env.APP_PUBLIC_URL || "http://127.0.0.1:8080").replace(/\/$/, "");
}
function formatAttachmentRow(att) {
  ...
  const fileUrl = documentId ? `/documents/${documentId}/download` : storedUrl.startsWith("/documents/") ? storedUrl : storedUrl;
  return {
    id: att.id,
    documentId,
...
async function buildEmailFileAttachments(pool, documentIds = []) {
  ...
  for (const docId of documentIds) {
    ...
    files.push({
      ...
      path: resolvedPath,
      contentType: doc.mime_type || void 0
    });
  }
  return files;
}
  ...
  async (req, res, next) => {
    try {
      ...
      const upsertLevel = async (agentUserId, employee, level, isPrimary, location, officeAddress) => {
        ...
        const notes = [location && `Location: ${location}`, officeAddress && `Office: ${officeAddress}`].filter(Boolean).join(" | ") || null;
        if (isPrimary) {
          await pool.execute(
            `UPDATE agent_employee_hierarchy SET is_primary = 0 WHERE agent_user_id = :agentId`,
            { agentId: agentUserId }
...
staffCommunicationRouter.get("/context", async (req, res, next) => {
  try {
    ...
    const hierarchyRows = req.auth.role === "agent" ? await fetchHierarchyMapping(pool, req.auth.userId) : [];
    res.json({
      role: req.auth.role,
      peer: peer ? {
        id: peer.peerId,
...
staffCommunicationRouter.post("/messages", async (req, res, next) => {
  try {
    ...
    const communicationEmail = peer?.communicationEmail || (await pool.execute(`SELECT email FROM user_profiles WHERE id = :id`, { id: input.peerId }))[0][0]?.email;
    ...
    for (const docId of documentIds) {
      ...
      if (!doc) continue;
      let canAttach = req.auth.role === "admin" || req.auth.role === "super_admin" || req.auth.role === "employee" && doc.assigned_employee_id === req.auth.userId;
      if (!canAttach && req.auth.role === "agent") {
        const scope = await resolveAgentScopeParams(pool, req.auth.userId);
        canAttach = doc.agent_id === req.auth.userId || scope.agent_code && doc.sourced_agent_code && doc.sourced_agent_code === scope.agent_code;
      }
    ...
    if (channel === "email" && communicationEmail) {
      ...
      const attachLines = attachments.length ? attachments.map((a) => {
        const link = a.documentId ? `${apiBase}/documents/${a.documentId}/download` : a.fileUrl?.startsWith("http") ? a.fileUrl : `${apiBase}${a.fileUrl?.startsWith("/") ? a.fileUrl : `/${a.fileUrl || ""}`}`;
      ...
      const emailHtml = `
        ...
        ${attachLines ? `<p><strong>Documents</strong></p><ul>${attachments.map((a) => {
        const link = a.documentId ? `${apiBase}/documents/${a.documentId}/download` : `${apiBase}${a.fileUrl || ""}`;
...
staffCommunicationRouter.get("/documents", async (req, res, next) => {
  try {
    ...
    const allowed = ["admin", "super_admin"].includes(req.auth.role) || req.auth.role === "employee" && app.assigned_employee_id === req.auth.userId || req.auth.role === "agent";
    if (!allowed) {
...
export {
  adminHierarchyRouter,
  staffCommunicationRouter
};
