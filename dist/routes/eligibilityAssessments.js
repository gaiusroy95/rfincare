...
const eligibilityAssessmentsRouter = Router();
...
eligibilityAssessmentsRouter.post("/", async (req, res, next) => {
  try {
    ...
    if (customerId) {
      ...
      const comparisonPath = loanType ? `/product-comparison?loanType=${encodeURIComponent(loanType)}#bank-comparison` : "/product-comparison#bank-comparison";
