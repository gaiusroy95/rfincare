...
const lenderPolicyImportRouter = Router();
const upload = multer({
    ...
    try {
      ...
      res.json(
        rows.map((row) => ({
          ...
          unsupportedSheets: typeof row.unsupported_sheets_json === "string" ? JSON.parse(row.unsupported_sheets_json) : row.unsupported_sheets_json,
