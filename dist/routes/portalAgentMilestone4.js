...
const portalAgentMilestone4Router = Router();
...
portalAgentMilestone4Router.post("/commission-bills", async (req, res, next) => {
  try {
    ...
    const wantPdf = String(req.query.format || req.body?.format || "").toLowerCase() === "pdf" || String(req.headers.accept || "").includes("application/pdf");
    if (wantPdf) {
    ...
    res.status(201).json({
      ...
      message: "Monthly commission bill generated. Download the PDF and email it to the Accounts Team for processing."
    });
  } catch (err) {
    next(err);
  }
