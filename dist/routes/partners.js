  ...
  async (req, res, next) => {
    try {
      ...
      const parsed = PartnerRegisterSchema.parse({
        ...
        addressLine2: bodyField(req.body, "addressLine2", "address_line2") || void 0,
      ...
      await notifyAdminsOfPartnerApplication(row).catch(
        (err) => console.warn("[partner-admin-email]", err?.message)
      );
      const inboundReferral = bodyField(req.body, "referralCode", "referral_code", "aref", "agentRef") || bodyField(req.query || {}, "aref", "ref");
      if (inboundReferral) {
      ...
      res.status(201).json({
        id,
        status: "pending",
        message: "Application submitted successfully. Our team will verify your documents and email you within 2–3 business days."
      });
    } catch (err) {
