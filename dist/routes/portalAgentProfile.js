...
const portalAgentProfileRouter = Router();
...
const BankOtpRequestSchema = z.object({
...
portalAgentProfileRouter.post("/email/confirm", async (req, res, next) => {
  try {
    ...
    if (!verified) return res.status(401).json({ error: "Invalid or expired OTP" });
    const newEmail = verified.payload?.newEmail === input.newEmail ? input.newEmail : input.newEmail;
