...
const loanCompareRouter = Router();
...
loanCompareRouter.post("/smart-recommendation", async (req, res, next) => {
  try {
    ...
    const priorities = input.priorities.map((p) => String(p).toLowerCase().replace(/-/g, "_")).filter((p) => SMART_LOAN_PRIORITIES.includes(p)).slice(0, 3);
