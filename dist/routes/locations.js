  ...
  try {
    res.json({
      data: await listTehsils({
        districtId: req.query.districtId || void 0,
        cityId: req.query.cityId || void 0
      })
    });
  } catch (err) {
    next(err);
  ...
  try {
    res.json({
      data: await listVillages({
        districtId: req.query.districtId || void 0,
        tehsilId: req.query.tehsilId || void 0
      })
    });
  } catch (err) {
    next(err);
...
locationsRouter.get(
  ...
  async (req, res, next) => {
    try {
      res.json({ data: await listServiceability({ bankId: req.query.bankId || void 0 }) });
    } catch (err) {
      next(err);
    }
  }
