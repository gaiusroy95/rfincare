...
const insuranceProductsRouter = Router();
...
function formatProductRow(row) {
  ...
  return {
    ...
    demographicMapping: typeof row.demographic_mapping === "object" ? row.demographic_mapping : row.demographic_mapping ? JSON.parse(row.demographic_mapping) : {},
  ...
  async (_req, res, next) => {
    try {
      ...
      res.json(rows.map((row) => ({
        ...
        requestConfig: typeof row.request_config === "object" ? row.request_config : row.request_config ? JSON.parse(row.request_config) : {},
        status: row.status
      })));
    } catch (err) {
      next(err);
