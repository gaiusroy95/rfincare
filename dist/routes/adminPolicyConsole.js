...
const adminPolicyConsoleRouter = Router();
  ...
  async (req, res, next) => {
    try {
      const rows = await listPolicyVersions({
        ...
        bankId: req.query.bankId || void 0,
    ...
    try {
      res.json({
        data: await listPolicyAudit({
          versionId: req.query.versionId || void 0,
          limit: req.query.limit
        })
      });
    } catch (err) {
    ...
    try {
      res.json({
        data: await listEligibilityRules({
          bankId: req.query.bankId || void 0,
          versionId: req.query.versionId || void 0,
          domain: req.query.domain || void 0
        })
      });
    } catch (err) {
      next(err);
  ...
  async (req, res, next) => {
    try {
      ...
      const rules = await listEligibilityRules({
        bankId: input.bankId || void 0,
        versionId: input.versionId || void 0
      });
      const applicant = {
      ...
      const bankTraces = (eligibility.banks || []).map((bank) => {
        ...
        const ruleResults = bankRules.map(
          (rule) => evaluateRule(rule, rule.conditions || [], applicant)
        );
