...
const portalAgentApplicationsRouter = Router();
...
async function resolveAgentMeta(pool, userId, { requireCode = false } = {}) {
  ...
  const agentCode = await ensureAgentCodeForUser(pool, userId) || row?.agent_code || null;
...
portalAgentApplicationsRouter.post("/applications", async (req, res, next) => {
  try {
    ...
    const body = req.body || {};
    const isEmployee = role === "employee" || body.submission_channel === "employee_assisted" || body.assisted_by_employee === true || body.assistedByEmployee === true;
  ...
  try {
    ...
    await pool.execute(
       ...
       VALUES (:id, :application_id, 'submitted', :message)`,
      {
        ...
        message: req.auth.role === "employee" ? "Application submitted by employee on behalf of customer" : "Application submitted by agent on behalf of customer"
      }
    );
    ...
    const leadId = parseJson(existing.data)?.lead_id || parseJson(existing.data)?.leadId || null;
    if (leadId) {
      try {
        await pool.execute(
          `UPDATE marketing_leads SET
