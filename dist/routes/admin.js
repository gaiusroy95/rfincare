...
adminRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const wantsDashboard = view === "dashboard" || view === "analytics" || hasMonth || hasRange;
      if (wantsDashboard) {
  ...
  async (req, res, next) => {
    try {
      ...
      const mapStaff = (row, role) => {
        ...
        const joiningLevel = role === "employee" && row.lead_level != null ? Number(row.lead_level) : null;
        return {
          id: row.id,
          role,
          name,
          ...
          label: joiningLevel ? `${code} — ${name} (L${joiningLevel})` : `${code} — ${name}`,
          joiningLevel,
