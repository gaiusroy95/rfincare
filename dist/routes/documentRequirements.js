...
const documentRequirementsRouter = Router();
  ...
  async (req, res, next) => {
    try {
      ...
      if (applicationId) {
        ...
        productType = productType || data.loan_purpose || data.loanPurpose || data.loan_type || data.loanType || null;
  ...
  async (_req, res, next) => {
    try {
      ...
      for (const docs of grouped.values()) {
        const first = docs[0];
        const documentTitles = docs.sort((a, b) => (Number(a.sort_order) || 0) - (Number(b.sort_order) || 0)).map((item) => item.title || item.document_type).filter(Boolean);
        const row = [
          first.bank_name || "",
          first.product_type || "",
          first.loan_type || "",
    ...
    try {
      ...
      if (productIdx < 0 || loanTypeIdx < 0 || docsStartIdx < 0) {
        return res.status(400).json({
          error: "CSV must include product_type, loan_type, and Documents needed columns."
        });
      }
      const pool = getPool();
