...
function oauthCookieOptions(maxAgeMs = 6e5) {
...
function setRefreshCookie(res, token) {
  ...
  res.cookie("refresh_token", token, {
    ...
    sameSite: "lax",
    path: "/auth/refresh",
    maxAge: Number(process.env.JWT_REFRESH_TTL_SECONDS || 60 * 60 * 24 * 30) * 1e3
  });
}
...
async function findOrCreateOAuthUser({ provider, providerUserId, email, fullName }) {
  ...
  if (email) {
    const eligibility2 = await checkCustomerEmailForOAuth(email);
    if (!eligibility2.allowed) {
      const err = new Error(
        eligibility2.reason === "not_registered" ? "No application found for this email. Please apply on Rfincare first, then sign in with Google." : "This email cannot be used for customer sign-in."
      );
      err.code = eligibility2.reason || "not_registered";
      throw err;
    }
...
oauthRouter.get("/:provider", async (req, res, next) => {
  try {
    ...
    if (!providerCfg?.enabled) {
      const returnOrigin2 = req.query.return_origin?.toString().trim();
      const frontendCallback2 = await resolveOAuthFrontendCallbackAsync(returnOrigin2);
      const front = new URL(frontendCallback2);
...
async function handleOAuthCallback(req, res, provider) {
  ...
  try {
    ...
    const code2 = err.code || "oauth_denied";
    return res.redirect(`${callbackUrl}?error=${encodeURIComponent(code2)}`);
  }
  if (profile.role !== "customer") {
