...
const approvalMatrixRouter = Router();
...
function pick(body, ...keys) {
  for (const key of keys) {
    if (body[key] !== void 0 && body[key] !== "") return body[key];
  }
  return void 0;
}
function normalizeRuleInput(body = {}) {
  ...
  return {
    ...
    isActive: body.is_active !== void 0 ? Boolean(body.is_active) : body.isActive !== void 0 ? Boolean(body.isActive) : true,
...
function formatRule(row) {
  ...
  return {
    ...
    tenure_unsecured_months: d.tenure_unsecured_months ?? d.tenureUnsecuredMonths ?? null,
    tenureUnsecuredMonths: d.tenure_unsecured_months ?? d.tenureUnsecuredMonths ?? null,
    tenure_secured_months: d.tenure_secured_months ?? d.tenureSecuredMonths ?? null,
    tenureSecuredMonths: d.tenure_secured_months ?? d.tenureSecuredMonths ?? null,
...
function canManageMatrix(role) {
  return hasPermission(role, "manage:approval_matrix") || hasPermission(role, "update:*") || hasPermission(role, "manage:*");
}
function canReadMatrix(role) {
  return hasPermission(role, "read:approval_matrix") || hasPermission(role, "read:*") || canManageMatrix(role);
}
...
async function listRulesForBank(pool, bankId) {
