...
const loanProductCatalogRouter = Router();
...
async function buildCatalogSlug(pool, { slugInput, label, bankId, category, excludeId = null }) {
  ...
  const preferred = bankId && category ? slugify(`${await getBankSlugPart(pool, bankId)}_${slugBase}_${category.slug}`) : slugBase;
...
loanProductCatalogRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const apiKey = body.bank_id ? bankProductApiKey(id) : await resolveUniqueCatalogApiKey(pool, body.api_key ? slugify(body.api_key) : slug);
      if (!slug || !apiKey) {
...
loanProductCatalogRouter.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const categoryId = body.category_id !== void 0 ? body.category_id : existing.category_id;
      const bankId = body.bank_id !== void 0 ? body.bank_id : existing.bank_id;
      ...
      const slug = slugInputChanged ? await buildCatalogSlug(pool, {
      ...
      const apiKey = bankId && !existing.bank_id ? bankProductApiKey(req.params.id) : existing.bank_id && !bankId ? await resolveUniqueCatalogApiKey(
        pool,
        body.api_key ? slugify(body.api_key) : slug,
        req.params.id
      ) : body.api_key != null ? slugify(body.api_key) : existing.api_key;
      const features = body.features !== void 0 ? parseFeatures(body.features) : parseFeatures(existing.features);
      await pool.execute(
         ...
         WHERE id = :id`,
        {
          ...
          is_active: body.is_active === void 0 ? null : toDbBool(body.is_active)
        }
      );
      if (category) {
        await ensureCategoryLandingCatalog(pool, category);
