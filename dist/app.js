...
import { contactInquiriesRouter } from "./routes/contactInquiries.js";
...
function createApp({ serveStatic = true } = {}) {
  ...
  app.use("/public/contact", contactInquiriesRouter);
  app.use("/api/public/contact", contactInquiriesRouter);
  ...
  app.use("/api/public/credit-card-savings", creditCardSavingsRouter);
  ...
  app.use("/public/loan-compare", loanCompareRouter);
  app.use("/api/public/loan-compare", loanCompareRouter);
  ...
  const buildPath = process.env.FRONTEND_DIST ? path.resolve(process.env.FRONTEND_DIST) : path.resolve(__dirname, "../../frontend/dist");
  app.use(express.static(buildPath));
  app.get("*", (req, res) => {
    if (req.path.startsWith("/api") || req.path.startsWith("/auth") || req.path.startsWith("/development-panel") || req.path.startsWith("/public") || req.path.startsWith("/uploads") || req.path.startsWith("/documents")) {
