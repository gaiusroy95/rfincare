...
async function provisionCustomerForAgent(pool, {
  email,
  phone,
  fullName,
  firstName,
  ...
  const displayName = fullName?.trim() || [firstName, lastName].filter(Boolean).join(" ").trim() || normalizedEmail;
  ...
  const userId = newId();
  const tempPassword = password || `RFC${String(phone || "").replace(/\D/g, "").slice(-4) || "0000"}Cust!`;
...
function calculateCommissionFromApplication(row, config) {
  const data = parseJson(row.data);
  const loanAmount = Number(
    data.requested_loan_amount || data.requestedLoanAmount || data.loan_amount || data.loanAmount || 0
  );
  return calculateCommissionFromAmount({ amount: loanAmount, config });
}
function calculateCommissionFromAmount({ amount, config } = {}) {
...
function commissionStatusForApplication(status) {
