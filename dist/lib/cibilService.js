...
function extractCibilDemographics({ application, customer, extra = {} } = {}) {
  const data = parseJson(application?.data);
  const personal = parseJson(data.personal) || {};
  const merged = { ...data, ...personal, ...extra };
  const name = extra.fullName || customer?.full_name || [merged.firstName || merged.first_name, merged.lastName || merged.last_name].filter(Boolean).join(" ") || firstValue(merged, ["fullName", "full_name", "name", "applicant_name"]);
  return {
    name,
...
async function listCibilVendors() {
...
async function updateCibilVendor(vendorKey, payload, updatedBy) {
...
async function resolveCustomerPullVendor(pool, customerId, { preferredVendorKeys = null } = {}) {
  const preferred = Array.isArray(preferredVendorKeys) ? preferredVendorKeys.map((k) => String(k || "").toLowerCase()).filter(Boolean) : [];
  if (preferred.length) {
    for (const key of preferred) {
      let vendor2 = await getVendorByKey(pool, key);
      if (!vendor2 && key === "experian") {
        await pool.execute(
        ...
        vendor2 = await getVendorByKey(pool, "experian");
      }
      if (vendor2) {
        return {
          vendor: vendor2,
...
async function productionPull({ vendor, application, customer, extra }) {
  if (!surepassConfigured(vendor)) {
    return {
      ...
      creditScore: null,
      reportPath: null,
      errorMessage: "Surepass credentials missing. Set SUREPASS_TOKEN or SUREPASS_ID_NUMBER + SUREPASS_PASSWORD (or save API key on the bureau vendor).",
      response: { error: "missing_credentials" }
    };
  }
  return surepassPull({ vendor, application, customer, extra });
}
async function pullCibilForApplication(applicationId, { forceSandbox = false } = {}) {
  await ensureMilestone4Schema();
  const pool = getPool();
  ...
  const result = useSandbox ? await sandboxPull({ vendor, application, customer: application }) : await productionPull({ vendor, application, customer: application });
  const checkId = newId();
  await pool.execute(
...
async function getLatestCibilCheck(applicationId) {
  await ensureMilestone4Schema();
  const pool = getPool();
  const [[row]] = await pool.execute(
...
async function requireSuccessfulCibilForSubmit(applicationId) {
  const latest = await getLatestCibilCheck(applicationId);
  if (!latest || latest.status !== "success") {
    const pull = await pullCibilForApplication(applicationId);
    if (pull.status !== "success") {
      const e = new Error(
        pull.errorMessage || "CIBIL verification failed. Application saved for manual review — contact operations."
      );
      e.status = 422;
...
async function getLatestCustomerCibilCheck(customerId) {
  await ensureMilestone4Schema();
  const pool = getPool();
  const [[row]] = await pool.execute(
...
async function pullCibilForCustomer(customerId, { forceSandbox = false, demographics = null, preferredVendorKeys = null } = {}) {
  await ensureMilestone4Schema();
  const pool = getPool();
  ...
  if (!vendor) {
    const e = new Error(
      vendorPlan.targetKey === "experian" ? "Experian vendor is not configured" : "No CIBIL vendor configured for first pull"
    );
    e.status = 400;
    throw e;
  }
  if (!forceSandbox && !vendorPlan.preferredOverride && !vendorPlan.isFirstPull && !vendorPlan.isFirstExperianRefresh) {
    const latest = await getLatestCustomerCibilCheck(customerId);
    if (latest?.checkedAt) {
      const daysSince = (Date.now() - new Date(latest.checkedAt).getTime()) / (1e3 * 60 * 60 * 24);
  ...
  if (demographics?.panNumber) {
    const existingData = (() => {
      try {
        return typeof stubApplication.data === "object" ? stubApplication.data : JSON.parse(stubApplication.data || "{}");
      } catch {
        return {};
      }
    })();
  ...
  const useSandbox = forceSandbox || Boolean(vendor.sandbox_mode);
  const result = useSandbox ? await sandboxPull({ vendor, application: stubApplication, customer }) : await productionPull({ vendor, application: stubApplication, customer });
  const checkId = newId();
  await pool.execute(
    `INSERT INTO cibil_checks
     (id, application_id, customer_id, vendor_key, status, credit_score, report_path, error_message, response_payload)
...
async function pullCibilForEmployee(demographics, employeeUserId, options = {}) {
  await ensureMilestone4Schema();
  const pool = getPool();
  const vendor = await getActiveVendor(pool);
  if (!vendor) {
  ...
  const reportUrlPrefix = options.reportUrlPrefix || (source === "admin_panel" ? "/admin/milestone4/cibil/report" : "/portal/employee/milestone4/cibil/report");
  const stubId = newId();
  const stubCustomer = {
    id: stubId,
  ...
  const result = useSandbox ? await sandboxPull({ vendor, application: stubApplication, customer: stubCustomer, extra }) : await productionPull({ vendor, application: stubApplication, customer: stubCustomer, extra });
  const checkId = newId();
  await pool.execute(
    `INSERT INTO cibil_checks
...
async function pullCibilForGuest(demographics, { upsertLead } = {}) {
  await ensureMilestone4Schema();
  const pool = getPool();
  const vendor = await getActiveVendor(pool);
  if (!vendor) {
...
async function getCibilCheckById(checkId) {
  await ensureMilestone4Schema();
  const pool = getPool();
  const [[row]] = await pool.execute(
    `SELECT cc.*, cv.display_name AS vendor_name
...
async function syncSurepassVendorFromEnv(pool = getPool()) {
...
export {
  extractCibilDemographics,
  getCibilCheckById,
  getLatestCibilCheck,
  getLatestCustomerCibilCheck,
  listCibilVendors,
  pullCibilForApplication,
  pullCibilForCustomer,
  pullCibilForEmployee,
  pullCibilForGuest,
  requireSuccessfulCibilForSubmit,
  syncSurepassVendorFromEnv,
  updateCibilVendor
};
