...
async function getAdminVerifierEmails() {
  ...
  const fromDb = [row?.verifier_email_1, row?.verifier_email_2, row?.verifier_email_3].map((e) => String(e || "").trim().toLowerCase()).filter(Boolean);
  if (fromDb.length) return fromDb.slice(0, 3);
  return fromEnv().slice(0, 3);
}
async function saveAdminVerifierEmails(emails, updatedBy) {
  await ensureAdminProfileSchema();
  const normalized = (emails || []).map((e) => String(e || "").trim().toLowerCase()).filter(Boolean).slice(0, 3);
...
async function collectOtpRecipientEmails(adminEmail) {
