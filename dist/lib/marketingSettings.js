...
const DEFAULTS = {
  ...
  seoDefaultDescription: "Rfincare - Simplifying loan applications with personalized financial solutions across India",
...
function formatPublicMarketingSettings(settings) {
  return {
...
async function getPublicMarketingSettings() {
  return formatPublicMarketingSettings(await getMarketingSettings());
}
async function updateMarketingSettings(input, updatedBy) {
  await ensureMarketingSchema();
  const pool = getPool();
...
async function logMarketingEvent({
  eventName,
  platform = "web",
  pagePath = null,
  utmSource = null,
...
async function getMarketingEventStats({ days = 30 } = {}) {
  await ensureMarketingSchema();
  const pool = getPool();
