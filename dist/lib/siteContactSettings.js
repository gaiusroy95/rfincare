...
let ensured = false;
const DEFAULTS = {
  tagline: "Intelligent loan matching that works for you. Transparency in every step. Your financial success is our mission.",
  ...
  branchAddress: "Shop no 3, 2nd Floor, Shiv Market, Near Kirtistambh circle, Ganganagar Road, Bikaner -334001 India",
  offices: [
    {
      title: "Reg. Office",
...
function parseOffices(value) {
  ...
  return arr.filter((o) => o && (o.title || o.address)).map((o) => ({
    title: o.title || "Office",
    address: o.address || ""
  }));
}
async function ensureSiteContactSchema() {
  ensured = true;
}
...
async function getSiteContactSettings() {
...
async function updateSiteContactSettings(input, updatedBy) {
  await ensureSiteContactSchema();
  const pool = getPool();
  ...
  const offices = input.offices?.length > 0 ? input.offices : [
    {
...
export {
  ensureSiteContactSchema,
  getSiteContactSettings,
  updateSiteContactSettings
};
