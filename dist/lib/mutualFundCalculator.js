...
function calculateMutualFundReturns(input = {}) {
  const mode = String(input.investmentMode || "sip").toLowerCase() === "lumpsum" ? "lumpsum" : "sip";
  const monthlyInvestment = Number(input.monthlyInvestment ?? 5e3);
  const lumpsumAmount = Number(input.lumpsumAmount ?? input.principal ?? 1e5);
