...
const REJECTED_AGENT_ONBOARDING_STATUSES = /* @__PURE__ */ new Set([
  "rejected",
  "suspended",
  "deactivated",
  "qc_rejected"
...
async function releaseRejectedAgentCredentials(pool, { email, username }) {
  ...
  const statusSql = [...REJECTED_AGENT_ONBOARDING_STATUSES].map((s) => `'${s.replace(/'/g, "''")}'`).join(", ");
  const conditions = [];
  const params = {};
  if (normalizedEmail) {
...
export {
  REJECTED_AGENT_ONBOARDING_STATUSES,
  releaseRejectedAgentCredentials
};
