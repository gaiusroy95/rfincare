...
function indiaNowParts(date = /* @__PURE__ */ new Date()) {
...
function parseRecipients(raw) {
  return String(raw || "").split(/[,;\s]+/).map((s) => s.trim().toLowerCase()).filter((s) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s));
}
...
function isScheduleDue(schedule, now = /* @__PURE__ */ new Date()) {
...
${buildCsvContent(section.columns, section.rows)}`;
    ...
    csv = `\uFEFFRfincare Master Report\r
Period: ${range.startDate} to ${range.endDate}\r
\r
${blocks.join("\r\n\r\n")}`;
...
async function sendScheduledReportEmail(pool, schedule, { force = false } = {}) {
...
async function runDueReportSchedules(pool, { limit = 50, forceAll = false } = {}) {
