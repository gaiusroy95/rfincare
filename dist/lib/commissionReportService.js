...
function computeRowCommission(row, config) {
  ...
  const gross = row.commission_amount != null ? Number(row.commission_amount) : rate ? Math.round(base * rate / 100) : 0;
...
async function buildAgentCommissionReport(agentId, filters = {}) {
...
function commissionReportToPdf(report) {
  const lines = [
    "Rfincare — Agent Commission Report",
    `Generated: ${report.generatedAt}`,
    "",
    ...report.entries.map(
      (e, i) => `${i + 1}. ${e.applicationNumber} | ${e.customerName} | ${e.loanType} | Status ${e.applicationStatus} | Comm ${e.commissionStatus} | Disbursed ${e.disbursedAmount} | Gross ${e.grossCommission} | TDS ${e.tdsAmount} | Net ${e.netPayout}`
    )
  ];
