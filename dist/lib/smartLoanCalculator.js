...
function outstandingPrincipal(principal, annualRate, tenureMonths, paidMonths) {
...
function scoreSmartLoanProduct(product, input) {
  ...
  if (priorities.has("low_roi")) {
    simulatedCost = round2(simulatedCost + totalInterest * 1e-3);
  }
  ...
  if (priorities.has("low_late_fee")) {
    const late = lateFeePct != null ? emiResult.emi * lateFeePct / 100 : toNum(lateFeeFixed, 0);
...
function rankSmartLoanProducts(products, input) {
  ...
  return {
    ...
    feeDisclaimer: usedDefaults ? "Some products are missing numeric foreclosure/part-payment fees in admin data. Default industry estimates were used for those fields — ask admin to fill Bank Product numeric charges for exact results." : null,
    input: {
