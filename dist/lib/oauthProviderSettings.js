...
async function ensureOauthProviderSchema() {
  ensured = true;
}
async function getOAuthGlobalSettings() {
  ...
  const envCallbacks = (process.env.OAUTH_FRONTEND_CALLBACK || "").split(",").map((s) => s.trim()).filter(Boolean);
...
async function getOAuthRedirectUri(provider) {
  const cfg = await getOAuthProviderConfig(provider);
  if (cfg?.redirectUri) {
...
async function updateOAuthSettings({ global: globalInput, providers: providerInputs }, updatedBy) {
  await ensureOauthProviderSchema();
  const pool = getPool();
  if (globalInput) {
    await pool.execute(
...
async function getOAuthAdminPayload() {
  const global = await getOAuthGlobalSettings();
...
async function getPublicOAuthConfig() {
  const global = await getOAuthGlobalSettings();
  const providers = await getOAuthProviderConfigs();
  const enabled = [];
...
export {
  ensureOauthProviderSchema,
  getOAuthAdminPayload,
  getOAuthCredentials,
  getOAuthGlobalSettings,
