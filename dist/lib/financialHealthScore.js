...
function buildFinancialHealthModel({
  activeLoans = [],
  verifiedDocs = 0,
  pendingDocs = 0,
  insurancePolicies = 0,
  ...
  const protectionScore = clampScore(
    (insurancePolicies > 0 ? 18 : insuranceLeadCount > 0 ? 8 : 0) + (insurancePolicies > 1 ? 2 : 0),
    20
  );
  const investmentScore = clampScore(
    (sipLeadCount > 0 ? 12 : 0) + (fixedDepositLeadCount > 0 ? 8 : 0) + (sipLeadCount > 1 ? 5 : 0),
    20
  );
  ...
  const profileScore = clampScore(
    (hasPhone ? 8 : 0) + (hasCustomerCode ? 7 : 0) + (hasEmail ? 5 : 0) + clampScore(verifiedDocs * 2, 10),
    25
  );
  ...
  if (protectionScore < 15) {
    improvementActions.push({
      ...
      description: insuranceLeadCount ? "You started an insurance enquiry — compare plans and buy online." : "Protect your family with life or health cover from top insurers.",
  ...
  const sortedActions = improvementActions.sort((a, b) => {
  ...
  return {
    ...
    nextBestAction,
    grade: totalScore >= 80 ? "Excellent" : totalScore >= 65 ? "Good" : totalScore >= 45 ? "Fair" : "Needs attention"
  };
}
export {
  buildFinancialHealthModel
};
