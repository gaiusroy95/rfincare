...
function getOtpInfrastructureStatus(providerConfig = {}) {
  ...
  return {
    msg91: {
      ...
      emailFrom: cfg.msg91EmailFromEmail || process.env.MSG91_EMAIL_FROM_EMAIL || process.env.MSG91_EMAIL_FROM || null,
      emailOtpTemplateId: cfg.msg91EmailOtpTemplateId || process.env.MSG91_EMAIL_OTP_TEMPLATE_ID || null
    },
    twilio: {
      configured: Boolean(
...
async function sendViaMsg91({ phone, otp, config }) {
  ...
  return sendMsg91Otp({
    ...
    config: {
      ...
      msg91OtpTemplateId: providerConfig.msg91OtpTemplateId || providerConfig.msg91TemplateId,
...
async function sendSmsOtp({ phone, otp, settings }) {
  ...
  if (provider === "console") {
    ...
    return {
      ...
      provider: "console",
      delivered: false,
      warning: "SMS operator is set to Console, so no SMS was actually sent. Set SMS operator to MSG91 or Twilio in Admin → OTP settings (and configure server env vars) to deliver OTP SMS."
    };
  }
  if (provider === "twilio") {
...
async function sendEmailOtp({ email, otp, settings }) {
  ...
  if (provider === "console") {
    ...
    return {
      ...
      provider: "console",
      delivered: false,
      warning: "Email operator is set to Console, so no email was actually sent. Set the email operator to SMTP or MSG91 in Admin → OTP settings (and configure the server credentials) to deliver OTP emails."
    };
  }
  if (provider === "smtp") {
    if (!smtpConfigured()) {
...
async function sendWhatsappOtp({ phone, otp, settings }) {
  ...
  if (provider === "console") {
    ...
    return {
      ...
      provider: "console",
      delivered: false,
      warning: "WhatsApp operator is set to Console, so no WhatsApp message was sent. Set WhatsApp operator to MSG91 in Admin → OTP settings."
    };
  }
  if (provider === "twilio") {
...
const CHANNEL_TIMEOUT_MS = Number(process.env.OTP_CHANNEL_TIMEOUT_MS || 2e4);
function channelTimeout(promise, label) {
  return withPromiseTimeout(
    promise,
    CHANNEL_TIMEOUT_MS,
    `${label} timed out after ${Math.round(CHANNEL_TIMEOUT_MS / 1e3)}s. Check server OTP/SMTP/MSG91 configuration.`
  );
}
function aggregateChannelErrors(results) {
  const failures = results.filter((r) => r.status === "rejected").map((r) => r.reason?.message || "Send failed");
...
async function sendOtpNotification({
  ...
  const tasks = [];
  const labels = [];
  const wantSms = channel === "sms" || channel === "both" || !channel && settings.requireMobileOtp;
  const wantEmail = channel === "email" || channel === "both" || !channel && settings.requireEmailOtp;
  const wantWhatsapp = channel === "whatsapp" || channel === "both" || !channel && settings.requireWhatsappOtp || Boolean(settings.requireWhatsappOtp) && Boolean(phone) && channel !== "email";
  if (wantSms && phone) {
...
async function sendDualChannelOtp({ email, phone, settings: settingsOverride }) {
  ...
  async function runSmsChannel() {
    try {
      ...
      const hint = settings.smsProvider === "msg91" ? `${errMsg} Confirm MSG91_AUTH_KEY, MSG91_OTP_TEMPLATE_ID / Admin OTP template, and a valid 6-char sender ID.` : errMsg;
      warnings.push(hint);
  ...
  } else if (!smsDelivered && phone && settings.whatsappProvider === "msg91" && isMsg91WhatsappConfigured(settings?.providerConfig)) {
    try {
      outcomes.whatsapp = await channelTimeout(
        sendWhatsappOtp({ phone, otp: mobileOtp, settings }),
        "WhatsApp OTP"
