...
function formatLoanAmount(data) {
  const amount = data.requested_loan_amount ?? data.requestedLoanAmount ?? data.loan_amount ?? data.loanAmount;
...
async function buildAgentRecentActivities(pool, { agentId, agentCode, commissionConfig = null, limit = 15 }) {
  const activities = [];
  ...
  try {
    ...
    for (const row of timeline || []) {
      ...
      pushActivity(activities, {
        ...
        description: row.message || `${row.customer_name || "Customer"} application moved to ${statusLabel.toLowerCase()} stage`,
        clientName: row.customer_name || "Customer",
        timestamp: row.created_at,
        metadata: {
          status: statusLabel,
  ...
  for (const row of approvedApps || []) {
    ...
    pushActivity(activities, {
      ...
      description: row.status === "disbursed" ? `${row.customer_name || "Customer"} loan disbursed` : `Payment processed for ${row.customer_name || "Customer"} ${loanTypeLabel(data)}`,
      clientName: row.customer_name || "Customer",
  ...
  try {
    ...
    for (const row of messages || []) {
      ...
      pushActivity(activities, {
        ...
        description: row.subject || String(row.body || "").slice(0, 120) || (row.customer_name ? `Communication regarding ${row.customer_name}` : "Staff message"),
