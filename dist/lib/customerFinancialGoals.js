...
async function ensureFinancialGoalsSchema(pool = getPool()) {
  try {
...
async function listFinancialGoals(customerId) {
  const pool = getPool();
...
function normalizeGoalInput(input = {}) {
  ...
  return {
    ...
    current,
    targetDate,
    notes: String(input.notes || "").trim().slice(0, 1e3) || null
  };
}
async function createFinancialGoal(customerId, input) {
  const pool = getPool();
  await ensureFinancialGoalsSchema(pool);
...
async function updateFinancialGoal(customerId, goalId, input) {
  const pool = getPool();
  await ensureFinancialGoalsSchema(pool);
  ...
  const data = normalizeGoalInput({
    ...
    targetDate: input.targetDate !== void 0 ? input.targetDate : existing.target_date,
...
async function deleteFinancialGoal(customerId, goalId) {
  const pool = getPool();
  await ensureFinancialGoalsSchema(pool);
