...
const CREDLAXMI_CATEGORY_CODES = new Set(CREDLAXMI_CATEGORIES.map((c) => c.code));
function getCredLaxmiCategory(code) {
  return CREDLAXMI_CATEGORIES.find((c) => c.code === String(code || "").toUpperCase()) || null;
}
function listCredLaxmiCategories({ expenseType } = {}) {
...
export {
  CREDLAXMI_CATEGORIES,
  CREDLAXMI_CATEGORY_CODES,
  getCredLaxmiCategory,
  listCredLaxmiCategories
};
