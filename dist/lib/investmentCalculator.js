...
const GOLD_TYPES = /* @__PURE__ */ new Set(["sovereign_gold_bonds", "digital_gold", "gold_etf", "silver_etf"]);
const BOND_TYPES = /* @__PURE__ */ new Set(["bonds", "corporate_bonds", "rbi_floating_bonds", "government_securities"]);
const INCOME_TYPES = /* @__PURE__ */ new Set(["reit", "invit"]);
function calculateInvestmentReturns(input = {}) {
  const type = String(input.calculatorType || "lump_sum").toLowerCase();
  const principal = Number(input.investmentAmount ?? input.principal ?? 1e5);
  const annualReturn = Number(input.annualReturn ?? input.expectedReturn ?? 8);
  const couponRate = Number(input.couponRate ?? annualReturn);
  const tenureYears = input.tenureYears != null && input.tenureYears !== "" ? Number(input.tenureYears) : input.tenureMonths != null && input.tenureMonths !== "" ? Number(input.tenureMonths) / 12 : 5;
