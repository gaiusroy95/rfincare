...
function pickFrontendOrigin(requestedOrigin) {
...
async function createResumeToken({ sessionKey, leadId, frontendOrigin, ttlDays = 7 }) {
  const pool = getPool();
...
async function resolveResumeToken(token) {
  const pool = getPool();
  ...
  if (new Date(row.expires_at) < /* @__PURE__ */ new Date()) return { error: "expired" };
...
async function ensureLeadDraftSession(pool, lead) {
  ...
  const nameParts = String(lead.full_name || "").trim().split(/\s+/).filter(Boolean);
  let eligibilityForm = {};
  if (lead.eligibility_data) {
    try {
      const parsed = typeof lead.eligibility_data === "object" ? lead.eligibility_data : JSON.parse(lead.eligibility_data);
...
async function upsertLeadFromDraft({ sessionKey, formData, loanType, currentStep, applicationId }) {
