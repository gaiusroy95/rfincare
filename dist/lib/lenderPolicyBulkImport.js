...
async function upsertProduct(conn, row, idMap) {
  ...
  const processingFee = feesForProduct && String(feesForProduct.Fee_Type || "").toLowerCase().includes("processing") ? feesForProduct.Calculation_Method === "PERCENTAGE" ? `${feesForProduct.Value}%` : `₹${feesForProduct.Value}` : void 0;
  ...
  const nextData = {
    ...
    minTenure: toNumber(row.Minimum_Tenure_Months) ? `${toNumber(row.Minimum_Tenure_Months)} months` : void 0,
    maxTenure: toNumber(row.Maximum_Tenure_Months) ? `${toNumber(row.Maximum_Tenure_Months)} months` : void 0,
    interestRate: toNumber(row.Interest_Rate_From) != null ? `${toNumber(row.Interest_Rate_From)}% - ${toNumber(row.Interest_Rate_To) ?? ""}%` : void 0,
    processingFee,
    maximum_foir_percentage: obligation ? toNumber(obligation.Maximum_FOIR_Percentage) : void 0,
