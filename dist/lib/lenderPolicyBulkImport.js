...
const SHEET_HEADER_SAMPLES = {
  ...
  Products: [
    {
      ...
      Minimum_Loan_Amount: 5e5,
      Maximum_Loan_Amount: 5e7,
  ...
  Income_Rules: [{ Rule_ID: "INC001", Product_ID: "PRD001", Minimum_Income: 25e3 }],
  ...
  Banking_Rules: [{ Rule_ID: "BNK001", Product_ID: "PRD001", Minimum_Average_Balance: 1e4 }],
...
function firstPresent(row, ...keys) {
  for (const key of keys) {
    if (row?.[key] !== void 0 && row?.[key] !== null && String(row[key]).trim() !== "") {
      return row[key];
    }
  }
  return void 0;
...
function resolvePolicyTemplatePath() {
  const candidates = [
...
function parsePolicyWorkbook(buffer) {
...
function buildImportPreview(parsed) {
  ...
  const lenderIds = /* @__PURE__ */ new Set();
  ...
  const decisionAllowed = /* @__PURE__ */ new Set(["PASS", "REVIEW", "FAIL"]);
...
async function saveValidatedImportJob({
  fileName,
  parsed,
  preview,
  createdBy
...
async function listImportJobs(limit = 30) {
  const pool = getPool();
  await ensureImportSchema(pool);
...
async function upsertProduct(conn, row, idMap) {
  ...
  const processingFeeRow = feesForProduct.find(
    (f) => String(f.Fee_Type || f.fee_type || "").toLowerCase().includes("processing")
  ) || null;
  ...
  const isPctFee = feeMethod === "PERCENTAGE" || feeMethod === "PERCENT" || feeMethod === "PCT" || feeMethod.includes("PERCENT");
  const isFixedFee = feeMethod === "FIXED" || feeMethod === "FLAT" || feeMethod === "AMOUNT" || feeMethod.includes("FIXED") || feeMethod.includes("FLAT");
  let processingFeePercentage = void 0;
  let processingFeeFixed = void 0;
  let processingFee = void 0;
  ...
  const resolvedMinMonths = minTenureMonths != null ? minTenureMonths : minTenureYears != null ? Math.round(minTenureYears * 12) : null;
  const resolvedMaxMonths = maxTenureMonths != null ? maxTenureMonths : maxTenureYears != null ? Math.round(maxTenureYears * 12) : null;
...
async function upsertDocumentRules(conn, sheets, idMap, result) {
  ...
  for (const [productKey, docRows] of Object.entries(byProduct)) {
    ...
    for (const doc of docRows) {
      ...
      const docType = String(doc.Document_Code || title).toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "").slice(0, 120) || "document";
      await conn.execute(
         ...
         )`,
        {
          ...
          title,
          subtitle: doc.Verification_Method ? `Verify: ${doc.Verification_Method}${doc.Conditional_Condition ? ` | ${doc.Conditional_Condition}` : ""}` : null,
...
function inferFieldFromRuleRow(row) {
  ...
  if (row.Field_Name || row.Attribute) {
    return {
      fieldKey: String(row.Field_Name || row.Attribute).toLowerCase().replace(/\s+/g, "_"),
...
function normalizeFoir(raw) {
  ...
  if (n > 1) return Math.round(n / 100 * 1e4) / 1e4;
  return n;
}
function resolveAnnualIncome(incomeRow) {
  ...
  if (min == null && monthlyMin != null) {
    min = monthlyMin < 1e5 ? monthlyMin * 12 : monthlyMin;
  }
  if (max == null && monthlyMax != null) {
    max = monthlyMax < 1e5 ? monthlyMax * 12 : monthlyMax;
  }
  return { min, max };
}
async function upsertApprovalMatrixFromSheets(conn, sheets, idMap, result) {
  ...
  for (const row of products) {
    ...
    const loanType = product.category?.loanType || product.data?.loan_type || mapProductCategory(row.Product_Category).loanType || "personal_loan";
    ...
    const tenureMonths = toNumber(firstPresent(tenure, "Maximum_Tenure_Months", "Max_Tenure_Months")) ?? toNumber(product.data?.max_tenure_months) ?? toNumber(row.Maximum_Tenure_Months);
    ...
    const data = {
      ...
      eligible_states: [],
      eligible_cities: [],
      min_loan_amount: toNumber(row.Minimum_Loan_Amount) ?? toNumber(product.data?.min_loan_amount) ?? null,
      max_loan_amount: toNumber(row.Maximum_Loan_Amount) ?? toNumber(product.data?.max_loan_amount) ?? null,
      min_age: minAge,
      max_age: maxAge,
      foir_unsecured: foir,
      foir_secured: foir,
...
async function upsertPolicyPackFromSheets(conn, sheets, idMap, result, committedBy) {
  ...
  if (matchingRows.length) {
    ...
    for (const row of matchingRows) {
      const key = String(row.Weight_Key || row.Factor || row.Rule_Name || "").toLowerCase().replace(/\s+/g, "_");
...
async function commitImportJob(jobId, committedBy) {
  const pool = getPool();
  await ensureImportSchema(pool);
  ...
  const sheets = typeof job.sheet_payload_json === "string" ? JSON.parse(job.sheet_payload_json) : job.sheet_payload_json || {};
  const prevResult = typeof job.commit_result_json === "string" ? (() => {
    try {
  ...
  const hasGeoRows = (sheets.Geo_Coverage || []).length > 0 || (sheets.Location_Rules || []).length > 0;
  if (job.status === "committed" && prevResult) {
    const geoSucceeded = Boolean(prevResult.geoVersionId) || prevResult.geoStatus === "skipped" || prevResult.geoStatus === "pending_approval" || !hasGeoRows;
    if (geoSucceeded) {
  ...
  const result = {
    ...
    documentsUpdated: 0,
    documentsSkipped: 0,
    unsupportedSheetsStored: (typeof job.unsupported_sheets_json === "string" ? JSON.parse(job.unsupported_sheets_json) : job.unsupported_sheets_json) || []
  };
  if (job.status !== "committed") {
    ...
    try {
      ...
      const wrapped = new Error(
        err?.message ? `Publish failed while saving lenders/products: ${err.message}` : "Publish failed while saving lenders/products"
      );
  ...
  try {
    ...
    if (geoRows.length) {
      ...
      const geo = await createGeoVersionFromSheetRows({
        ...
        sourceJobId: jobId,
        changeReason: geoRows.find((r) => r.Change_Reason)?.Change_Reason || "Bulk upload Geo_Coverage / Location_Rules",
      ...
      result.geoNote = "Geo version created as pending_approval — Super Admin must approve before live eligibility uses it.";
    } else {
    ...
    result.geoNote = "Lenders/products/rules were published, but geo Location_Rules import failed. Click Publish import again to retry geo only, or fix lender codes under Lender geo policy.";
  }
  try {
    await pool.execute(
      `UPDATE lender_policy_import_jobs SET
...
function buildPolicyTemplateWorkbook() {
...
function buildErrorReportCsv(errorReport) {
