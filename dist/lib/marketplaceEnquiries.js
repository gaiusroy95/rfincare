const MARKETPLACE_ENQUIRY_TYPES = /* @__PURE__ */ new Set(["insurance", "mutual_funds"]);
...
function formatProductLabel(loanType, eligibilityData) {
  ...
  if (eligibilityData?.productCategory) {
    return String(eligibilityData.productCategory).replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
  }
  return MARKETPLACE_TYPE_LABELS[loanType] || String(loanType || "Enquiry").replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
}
...
function isMarketplaceProductFilter(loanType) {
...
function buildMarketplaceEnquiryQuery(filters = {}) {
...
function formatMarketplaceEnquiryAsApplication(row) {
...
async function fetchMarketplaceEnquiries(pool, filters = {}, { limit = 200 } = {}) {
...
async function countMarketplaceEnquiries(pool, filters = {}) {
  const { where, params } = buildMarketplaceEnquiryQuery(filters);
...
export {
  buildMarketplaceEnquiryQuery,
  countMarketplaceEnquiries,
  fetchMarketplaceEnquiries,
  formatMarketplaceEnquiryAsApplication,
