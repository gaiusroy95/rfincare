...
async function fetchEmployeeAccessControls(employeeUserId) {
...
async function fetchEmployeeAccessControlsMap(employeeUserIds = []) {
...
function buildEffectiveEmployeeAccess(rows = []) {
  ...
  const grantedModuleLabels = Object.keys(modules).map((key) => MODULE_LABELS[key] || key).filter(Boolean);
  return {
    configured: true,
    isActive: anyActive,
    expired: false,
...
async function getEffectiveEmployeeAccess(employeeUserId) {
  const rows = await fetchEmployeeAccessControls(employeeUserId);
  return buildEffectiveEmployeeAccess(rows);
}
function employeeHasModulePermission(access, moduleName, permission = "read") {
...
function requireEmployeeModuleAccess(access, moduleName, permission = "read") {
  if (!access) return;
...
async function assertEmployeeAccess(req, moduleName, permission = "read") {
