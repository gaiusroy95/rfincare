...
function mapPartnerRegistrationRow(row) {
  if (!row) return null;
...
async function notifyAdminsOfPartnerApplication(registration) {
  const pool = getPool();
...
async function approvePartnerRegistration(registrationId, reviewerUserId) {
...
async function rejectPartnerRegistration(registrationId, reviewerUserId, reason) {
  await ensurePartnerRegistrationSchema();
  const pool = getPool();
  const [[reg]] = await pool.execute(
    `SELECT * FROM partner_registrations WHERE id = :id LIMIT 1`,
