...
async function getProviderConfigByCode(pool, providerCode) {
...
async function createInsuranceCheckout(rawInput) {
...
async function fetchInsuranceQuote({ productId, customer, demographics, coverage }) {
  const pool = getPool();
...
async function createInsuranceProposal({ purchaseOrderId, publicToken, customer, demographics, quoteId, returnUrl }) {
  const pool = getPool();
...
async function getInsurancePurchaseById(id, publicToken) {
  const pool = getPool();
  const [[row]] = await pool.execute(
...
function resolveInsuranceIntegrationMeta(providerConfig) {
...
async function getInsurancePurchaseStatus(id, publicToken) {
  const row = await getInsurancePurchaseById(id, publicToken);
  if (!row) return null;
  const pool = getPool();
  const providerConfig = row.insurer_provider_code ? await getProviderConfigByCode(pool, row.insurer_provider_code) : null;
...
async function completeInsurancePurchasePush(orderId) {
  const pool = getPool();
  const [[order]] = await pool.execute(
...
async function markInsuranceOrderPaid({
  razorpayOrderId,
  razorpayPaymentId,
  razorpaySignature,
  eventPayload = null
  ...
  if (paidOrder) {
    const agentCode = paidOrder.sourced_agent_code || (() => {
      try {
        const demo = typeof paidOrder.demographic_data === "string" ? JSON.parse(paidOrder.demographic_data || "{}") : paidOrder.demographic_data || {};
...
export {
  ...
  getProviderConfigByCode,
  markInsuranceOrderPaid,
  resolveInsuranceIntegrationMeta
};
