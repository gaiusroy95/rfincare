...
function stepUpSipResult(input) {
  const base = toNum(input.monthlyInvestment, 5e3);
...
function incomeTaxResult(input) {
  ...
  return {
    ...
    summary: regime === "both" ? `Recommended: ${recommended} regime (saves ₹${round2(Math.abs(oldTax.totalTax - newTax.totalTax)).toLocaleString("en-IN")})` : `Tax under ${regime} regime: ₹${(regime === "old" ? oldTax : newTax).totalTax.toLocaleString("en-IN")}`
  };
}
...
function section80dResult(input) {
  const selfLimit = 25e3;
  const parentLimit = input.parentsSenior ? 5e4 : 25e3;
...
function bondYieldResult(input) {
  const face = toNum(input.faceValue, 1e3);
...
function runCalculator(slug, input = {}) {
...
import { CALCULATOR_CATEGORIES, CALCULATOR_REGISTRY, getCalculatorBySlug as getCalculatorBySlug2, listCalculators } from "./registry.js";
export {
  CALCULATOR_CATEGORIES,
  CALCULATOR_REGISTRY,
  getCalculatorBySlug2 as getCalculatorBySlug,
  listCalculators,
  runCalculator
};
