...
function calculateEmi(principal, annualRate, tenureMonths) {
  ...
  if (r === 0) {
    const emi2 = p / n;
    return { emi: round2(emi2), totalPayment: round2(emi2 * n), totalInterest: 0, tenureMonths: n };
  }
  const factor = (1 + r) ** n;
...
function swpCorpus(monthlyWithdrawal, annualReturnPercent, years) {
...
function fdMaturity(principal, annualRate, years, compounding = 4) {
  const p = toNum(principal);
...
function rdMaturity(monthlyDeposit, annualRate, months) {
...
function ppfMaturity(annualDeposit, annualRate, years) {
...
function compoundInterest(principal, annualRate, years, frequency = 1) {
  const p = toNum(principal);
  const r = toNum(annualRate) / 100;
...
function inflationAdjustedValue(futureValue, inflationRate, years) {
  const fv = toNum(futureValue);
  const inf = toNum(inflationRate) / 100;
  const t = toNum(years, 1);
  return round2(fv / (1 + inf) ** t);
}
function retirementCorpus(monthlyExpense, inflationRate, yearsToRetirement, postRetirementYears, returnRate) {
...
function pensionPayout(corpus, annualRate, payoutYears) {
...
function npsProjection(age, retirementAge, monthlyContribution, employerContribution, expectedReturn) {
...
function annuityPayout(principal, annualRate, payoutYears) {
  const p = toNum(principal);
  const r = toNum(annualRate) / 100 / 12;
...
function gstAmount(amount, gstRate, inclusive = false) {
  const base = toNum(amount);
  const rate = toNum(gstRate, 18) / 100;
  if (inclusive) {
    const taxable = base / (1 + rate);
    const gst2 = base - taxable;
    return { taxableValue: round2(taxable), gstAmount: round2(gst2), total: round2(base) };
  }
  const gst = base * rate;
  return { taxableValue: round2(base), gstAmount: round2(gst), total: round2(base + gst) };
}
function incomeTaxOldRegime(annualIncome, deductions = 0) {
...
function incomeTaxNewRegime(annualIncome) {
...
function hraExemption(basicSalary, hraReceived, rentPaid, metro = true) {
...
function capitalGain(purchasePrice, salePrice, holdingMonths, assetType = "equity") {
  const gain = toNum(salePrice) - toNum(purchasePrice);
  const isLtcg = holdingMonths >= (assetType === "equity" ? 12 : 24);
  let tax = 0;
  if (assetType === "equity") {
    tax = isLtcg ? Math.max(0, gain - 125e3) * 0.125 : gain * 0.2;
  } else {
...
function loanEligibility(monthlyIncome, existingEmi, interestRate, tenureYears, foir = 0.5) {
...
function debtConsolidation(loans, newRate, newTenureYears) {
...
function goalSip(goalAmount, years, expectedReturn) {
...
function cagr(beginValue, endValue, years) {
  ...
  const cagr2 = ((e / b) ** (1 / t) - 1) * 100;
  return { cagrPercent: round2(cagr2) };
}
function gratuity(basicSalary, yearsOfService, isCovered = true) {
  const basic = toNum(basicSalary);
...
function epfCorpus(monthlyBasic, employeePct, employerPct, annualIncrease, years, epfRate) {
  let balance = 0;
...
function fireNumber(annualExpense, withdrawalRate = 4) {
...
function rentVsBuy(rent, homePrice, downPayment, loanRate, tenureYears, appreciation) {
