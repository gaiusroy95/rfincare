...
function googleCalendarConfigured() {
  return Boolean(
    process.env.GOOGLE_CALENDAR_CLIENT_EMAIL && privateKeyFromEnv() && (process.env.GOOGLE_CALENDAR_ID || "primary")
  );
}
...
async function createGoogleCalendarEvent(event) {
  ...
  const attendees = (event.attendeeEmails || []).filter(Boolean).map((email) => ({ email }));
  const payload = {
