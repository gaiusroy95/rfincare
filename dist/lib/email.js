function smtpConfigured() {
  return Boolean(
    process.env.SMTP_HOST && (process.env.SMTP_FROM || process.env.SMTP_FROM_EMAIL || process.env.SMTP_USER)
  );
}
...
async function sendStaffWelcomeEmail({ email, fullName, role, password, loginPath }) {
...
async function sendPartnerApplicationAdminEmail({ recipients, applicant }) {
  const appUrl = process.env.APP_PUBLIC_URL || process.env.API_PUBLIC_URL || "http://127.0.0.1:4028";
...
async function sendPartnerWelcomeEmail({
  email,
  fullName,
  username,
  password,
...
async function sendPartnerRejectionEmail({ email, fullName, reason }) {
