...
function normalizeStoredUploadName(storedPath) {
  const key = normalizeStorageKey(storedPath);
  if (!key) return null;
  return basename(key);
}
function resolveUploadFilePath(storedPath, extraNames = []) {
...
async function streamStoredUpload(storedPath, extraNames = []) {
...
async function uploadExists(storedPath, extraNames = []) {
  const keys = /* @__PURE__ */ new Set();
  for (const value of [storedPath, ...extraNames]) {
    const key = normalizeStorageKey(value);
    if (key) keys.add(key);
