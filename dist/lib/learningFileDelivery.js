...
function normalizeLearningPublicUrl(fileUrl) {
  ...
  for (const dirPath of searchDirs) {
    ...
    try {
      for (const entry of readdirSync(dirPath)) {
        if (entry === fileName || entry.toLowerCase() === lower || entry.toLowerCase().endsWith(`-${lower}`)) {
...
function resolveLearningDiskPath({ filePath, fileUrl, fileName } = {}) {
  const names = [
    fileName,
...
function resolveLearningOpenTarget({
  id,
  contentType,
  videoUrl,
  fileUrl,
...
function buildConfigCircularId(fileUrl) {
  return `cfg-${Buffer.from(String(fileUrl)).toString("base64url").slice(0, 12)}`;
}
export {
  buildAgentCircularDownloadPath,
  buildAgentContentDownloadPath,
  buildConfigCircularId,
  buildEmployeeContentDownloadPath,
