...
function buildAgentPerformanceAnalytics(apps, commissionEntries) {
  ...
  for (const range of ranges) {
    ...
    performanceAnalytics[range] = buckets.map(
      (b) => aggregateBucket(apps, commissionById, b)
    );
  }
...
function buildAgentMetricTrends(apps, commissionEntries) {
  const now = /* @__PURE__ */ new Date();
