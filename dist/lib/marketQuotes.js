...
const MEMORY_TTL_MS = Number(process.env.MARKET_OVERVIEW_CACHE_MS || 6e4);
...
async function fetchUsdInrLive() {
  try {
    ...
    const rate2 = Number(data2?.usd?.inr);
    if (Number.isFinite(rate2)) return { rate: rate2, updatedAt: data2?.date || null };
  } catch {
  }
...
function upsertSeriesHistory(history, value, keyDate = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10)) {
...
async function fetchLiveOverview() {
  ...
  if (sensex && sensexHist.length < 3) {
    sensexHist = await safe("sensex history", () => fetchMoneycontrolHistory("in;SEN"), errors) || [];
  }
  if (nifty) {
    indices.push(
      buildIndexRow({
  ...
  if (gold && Number.isFinite(usdInr)) {
    ...
    const prev = goldHistory.length >= 2 ? goldHistory[goldHistory.length - 2].value : disk?.goldPrevClose;
    const changePct = Number.isFinite(Number(prev)) && Number(prev) > 0 ? (goldPerGram - Number(prev)) / Number(prev) * 100 : 0;
  ...
  if (Number.isFinite(usdInr)) {
    ...
    const prev = usdHistory.length >= 2 ? usdHistory[usdHistory.length - 2].value : disk?.usdInrPrev;
    const changePct = Number.isFinite(Number(prev)) && Number(prev) > 0 ? (usdInr - Number(prev)) / Number(prev) * 100 : 0;
    indices.push(
      buildIndexRow({
        id: "usdinr",
        name: "USD/INR",
  ...
  const payload = {
    ...
    indices,
    warnings: errors.length ? errors : void 0
  };
  writeDiskCache({
    ...disk,
    lastPayload: payload,
...
async function getMarketOverview({ force = false } = {}) {
  const now = Date.now();
