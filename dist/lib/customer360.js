...
async function buildCustomer360(pool, customerId, email) {
  ...
  const abandonedCheckouts = insuranceOrders.filter(
    (o) => ["created", "pending_payment"].includes(o.payment_status)
  );
  const abandonedSips = sipOrders.filter((o) => {
    ...
    return updated > 0 && Date.now() - updated > 60 * 60 * 1e3;
  });
  const activeSips = sipOrders.filter(
    (o) => ["mandate_pending", "active"].includes(o.status)
  );
  const activePolicies = insuranceOrders.filter(
    (o) => o.payment_status === "paid" || o.insurer_push_status === "pushed"
  );
  const renewalDue = activePolicies.filter((o) => {
    const ref = o.paid_at || o.created_at;
    if (!ref) return false;
    const months = (Date.now() - new Date(ref).getTime()) / (1e3 * 60 * 60 * 24 * 30);
    return months >= 11;
  });
  const openEnquiries = marketingLeads.filter(
    (l) => !["assigned", "converted"].includes(l.status)
  );
  const journeys = [];
  ...
  for (const lead of openEnquiries.slice(0, 3)) {
    ...
    journeys.push({
      ...
      description: data?.productLabel ? `You enquired about ${data.productLabel}.` : "Pick up where you left off in the marketplace.",
      path: lead.loan_type === "insurance" ? "/insurance-marketplace" : lead.loan_type === "mutual_funds" ? "/mutual-fund-marketplace" : "/customer-dashboard",
      priority: 70,
      status: lead.status
    });
  }
