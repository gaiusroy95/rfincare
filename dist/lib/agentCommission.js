...
const AGENT_COMMISSION_CSV_HEADERS = [
  "agent_name",
  "agent_code",
  "mobile_number",
  "account_status",
...
function normalizeCommissionCsvRow(raw) {
  ...
  const loanType = LOAN_TYPE_ALIASES[loanKey] || (loanKey.endsWith("_loan") ? loanKey : loanKey);
...
async function resolveCircularFileUrl(pool, row, circularFilesByName) {
  ...
  if (uploadVal) {
    const matched = circularFilesByName[uploadVal] || circularFilesByName[uploadVal.toLowerCase()] || circularFilesByName[uploadVal.split(/[/\\]/).pop()] || circularFilesByName[uploadVal.split(/[/\\]/).pop()?.toLowerCase()];
...
function resolveCircularUploadMeta(row, circularFilesByName) {
  ...
  return circularFilesByName[uploadVal] || circularFilesByName[uploadVal.toLowerCase()] || circularFilesByName[uploadVal.split(/[/\\]/).pop()] || circularFilesByName[uploadVal.split(/[/\\]/).pop()?.toLowerCase()] || null;
}
async function ensureCommissionCircularRecord(pool, { title, fileUrl, fileName, filePath, uploadedBy }) {
...
async function fetchAgentCommissionCirculars(pool) {
...
async function upsertAgentCommissionConfig(pool, agentUserId, row, { updatedBy, circularFilesByName = {} }) {
...
async function importAgentCommissionRows(pool, rawRows, { updatedBy, circularFilesByName = {} }) {
  const results = [];
  let imported = 0;
  let failed = 0;
...
function buildAgentCommissionTemplateCsv() {
  const sample = [
    "Sample Agent",
    "RFA20261",
    "9876543210",
