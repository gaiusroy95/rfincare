...
const LEGAL_PAGE_BY_SLUG = new Map(LEGAL_PAGE_CATALOG.map((p) => [p.slug, p]));
function getCatalogLegalPage(slug) {
  return LEGAL_PAGE_BY_SLUG.get(String(slug || "").trim()) || null;
}
function listCatalogLegalPages() {
...
async function ensureLegalPages(pool) {
  ...
  for (const page of LEGAL_PAGE_CATALOG) {
    ...
    const isStub = !body.trim() || /content managed by admin\.?/i.test(body) || /^<p>\s*[^<]{0,80}content managed by admin[^<]{0,40}\s*<\/p>$/i.test(body.trim());
    if (isStub) {
      await pool.execute(
        `UPDATE legal_pages
...
async function getLegalPageBySlug(pool, slug, { createIfMissing = true } = {}) {
...
async function upsertLegalPage(pool, { slug, title, bodyHtml, updatedBy }) {
  const normalized = String(slug || "").trim();
  const catalog = getCatalogLegalPage(normalized);
