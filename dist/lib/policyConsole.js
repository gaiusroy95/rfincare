...
async function ensurePolicyConsoleSchema(pool = getPool()) {
...
async function listPolicyVersions({ status, bankId, limit = 100 } = {}) {
...
async function getPolicyVersion(id) {
  await ensurePolicyConsoleSchema();
  const pool = getPool();
  const [rows] = await pool.query(
    `SELECT v.*, b.name AS bank_name, bp.name AS product_name
  ...
  return {
    ...
    rules: rules.map((r) => ({
      ...
      conditions: conditions.filter((c) => c.rule_id === r.id).map((c) => ({
        ...c,
...
async function submitVersion(id, actorId, reason) {
  return transitionVersion(id, {
    fromStatuses: ["draft", "rejected"],
    toStatus: "submitted",
    actorId,
    reason,
    extraSets: { submitted_by: actorId, submitted_at: (/* @__PURE__ */ new Date()).toISOString() }
  });
}
async function approveVersion(id, actorId, reason) {
  return transitionVersion(id, {
    fromStatuses: ["submitted"],
    toStatus: "approved",
    actorId,
    reason,
    extraSets: { approved_by: actorId, approved_at: (/* @__PURE__ */ new Date()).toISOString() }
  });
}
async function rejectVersion(id, actorId, reason) {
  return transitionVersion(id, {
...
async function publishVersion(id, actorId, reason) {
  await ensurePolicyConsoleSchema();
  const pool = getPool();
  const [rows] = await pool.query(`SELECT * FROM product_policy_versions WHERE id = :id`, { id });
  const version = rows[0];
  ...
  try {
    ...
    if (version.bank_product_id) {
      ...
      snap.policy_published_at = (/* @__PURE__ */ new Date()).toISOString();
      await conn.execute(
...
async function activateVersionPolicyData(conn, version) {
  ...
  const productClause = version.bank_product_id ? "AND (bank_product_id = :pid OR bank_product_id IS NULL)" : "AND bank_product_id IS NULL";
  for (const table of [
    "eligibility_rules",
    "property_ltv_rules",
    "dscr_rules",
...
async function listPolicyAudit({ versionId, limit = 100 } = {}) {
  await ensurePolicyConsoleSchema();
  const pool = getPool();
  const params = { limit: Math.min(Number(limit) || 100, 500) };
  let where = "";
  ...
  for (let i = 0; i < conditions.length; i += 1) {
    ...
    await db.execute(
       ...
       )`,
      {
        ...
        value_to_json: c.valueTo != null || c.value_to_json != null ? JSON.stringify(c.valueTo ?? c.value_to_json) : null,
...
async function listEngineEligibilityRules({ bankId } = {}) {
  await ensurePolicyConsoleSchema();
  const pool = getPool();
  const params = {};
  let bankFilter = "";
  ...
  return rules.map((r) => ({
    ...r,
    source_row_json: parseJson(r.source_row_json),
    conditions: conds.filter((c) => c.rule_id === r.id).map((c) => ({
      ...c,
      value_json: parseJson(c.value_json),
      value_to_json: parseJson(c.value_to_json)
    }))
...
async function listEligibilityRules({ bankId, versionId, domain } = {}) {
  await ensurePolicyConsoleSchema();
  const pool = getPool();
