...
const BANK_PARTNERSHIP_COLUMNS = [
  "bank_name",
  "status",
  "commission_slab",
  "submitted_cases",
...
async function buildBankPartnershipRows(pool, { start, end }) {
  ...
  const rows = [...byBank.values()].map((g) => {
    ...
    const avgDays = g.processingDays.length > 0 ? Math.round(g.processingDays.reduce((a, b) => a + b, 0) / g.processingDays.length * 10) / 10 : null;
    return {
...
export {
  BANK_PARTNERSHIP_COLUMNS,
  buildBankPartnershipRows
};
