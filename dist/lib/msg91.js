...
function getMsg91EmailConfig(overrides = {}) {
  ...
  return {
    authKey,
    domain: overrides.msg91EmailDomain || process.env.MSG91_EMAIL_DOMAIN || "",
...
function isMsg91EmailConfigured(overrides = {}) {
...
async function sendMsg91TransactionalSms({
  phone,
  message,
  otp,
  config: overrides = {},
  ...
  requireAuthKey(config);
  const mobile = normalizeIndianMobile(phone);
  const tpl = messageTemplate || "Your Rfincare verification code is {{otp}}. Valid for 10 minutes.";
...
async function sendMsg91EmailOtp({
  ...
  const res = await fetchWithTimeout(
    MSG91_EMAIL_URL,
    {
      ...
      timeoutMessage: "MSG91 email request timed out. Verify MSG91 email domain and template on the server."
    },
    MSG91_FETCH_TIMEOUT_MS
  );
  await parseMsg91Response(res);
