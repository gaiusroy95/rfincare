...
async function resolveLiveCibilForCustomerEligibility(customerId, input = {}) {
  ...
  if (profile.bureauScore && profile.bureauCheckedAt && !refreshRequested) {
    const daysSince = (Date.now() - new Date(profile.bureauCheckedAt).getTime()) / (1e3 * 60 * 60 * 24);
...
export {
  resolveLiveCibilForCustomerEligibility
};
