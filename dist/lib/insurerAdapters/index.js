...
async function genericApiPurchase({ providerConfig, purchaseOrder, payload }) {
  ...
  const res = await axios.post(url, payload, {
    ...
    timeout: Number(requestConfig.timeoutMs || 3e4)
  });
  return {
    ok: true,
    externalReferenceId: res.data?.policyId || res.data?.policyNumber || res.data?.orderId || res.data?.referenceId || null,
...
async function pushPurchaseToInsurer({ providerConfig, purchaseOrder, payload }) {
...
async function demoProposal({ payload }) {
  return {
    ...
    paymentUrl: payload?.returnUrl ? `${payload.returnUrl}?status=success&demo=1` : "https://example.com/payment"
  };
}
...
async function fetchQuoteFromProvider({ providerConfig, payload }) {
  const mode = String(providerConfig?.integration_mode || "generic_api").toLowerCase();
...
async function createProposalWithProvider({ providerConfig, payload }) {
  const mode = String(providerConfig?.integration_mode || "generic_api").toLowerCase();
...
export {
  createProposalWithProvider,
  fetchQuoteFromProvider,
  pushPurchaseToInsurer
};
