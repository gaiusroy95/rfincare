  ...
  try {
    ...
    for (const raw of rows || []) {
      const lenderKey = String(
        raw.Lender_Code || raw.Lender_ID || raw.lender_code || raw.Bank_Code || raw.bank_code || ""
      ).trim();
      if (!lenderKey) continue;
      if (resolvedBanks[lenderKey] || resolvedBanks[lenderKey.toUpperCase()] || raw.bank_id) {
        continue;
      }
    ...
    for (const raw of rows || []) {
      ...
      const bankId = resolvedBanks[lenderKey] || resolvedBanks[lenderKey.toUpperCase()] || raw.bank_id || null;
      if (!bankId) {
        skippedNoBank += 1;
        continue;
      }
    ...
    for (let i = 0; i < pending.length; i += BATCH_SIZE) {
      ...
      const valuesSql = chunk.map(
        (_row, idx) => `(:id_${idx}, :version_id_${idx}, :bank_id_${idx}, :geo_level_${idx}, :state_name_${idx}, :district_name_${idx}, :tehsil_name_${idx}, :pincode_${idx}, :coverage_type_${idx}, :branch_code_${idx}, :radius_km_${idx}, :remarks_${idx}, :priority_${idx})`
      ).join(",\n");
      const params = {};
