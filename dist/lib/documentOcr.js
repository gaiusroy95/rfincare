...
function extractFieldsFromText(text, documentType = "") {
  ...
  const aadhaar = raw.replace(/\s/g, "").match(/\b([0-9]{12})\b/) || raw.match(/\b([0-9]{4}\s[0-9]{4}\s[0-9]{4})\b/);
...
async function extractTextFromBuffer(buffer, mimeType = "") {
  ...
  if (mime.includes("pdf")) {
    try {
      ...
      const text = typeof textResult === "string" ? textResult : textResult?.text || Object.values(textResult || {}).filter((v) => typeof v === "string").join("\n");
  ...
  return { text: buffer.toString("utf8", 0, Math.min(buffer.length, 5e4)), engine: "buffer-scan" };
}
async function ensureOcrColumns(pool = getPool()) {
...
async function runDocumentOcr(documentId, { profile = {}, extractedText = null } = {}) {
  const pool = getPool();
  ...
  const confidence = ocrConfidence != null ? Number(((ocrConfidence + score.confidence) / 2).toFixed(4)) : Number(score.confidence.toFixed(4));
  const payload = {
    textPreview: String(text || "").slice(0, 4e3),
    fields,
    checks: score.checks,
