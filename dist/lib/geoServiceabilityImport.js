...
const VALID_STATUS = /* @__PURE__ */ new Set(["serviceable", "restricted", "not_serviceable"]);
...
function sheetToRows(buffer, filename = "") {
  ...
  if (name.endsWith(".csv")) {
    ...
    return XLSX.utils.sheet_to_json(sheet2, { defval: "" });
  }
...
function buildServiceabilityTemplateWorkbook() {
...
function parseServiceabilityUpload(buffer, filename) {
...
async function importServiceabilityForBank({ bankId, rows, replaceExisting = false }) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const notesParts = [
        row.notes,
        row.state || row.district || row.city || row.locality ? [row.locality, row.city, row.district, row.state].filter(Boolean).join(", ") : ""
      ].filter(Boolean);
