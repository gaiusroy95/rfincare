...
function buildApplicationPdfLines({ row, data, documents, consents }) {
  const submittedAt = row.submitted_at ? new Date(row.submitted_at).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" }) : (/* @__PURE__ */ new Date()).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });
  const lines = [
    ...
    `Requested amount: ${formatInr(
      field(data, "loanAmount", "loan_amount") ?? field(data, "requestedLoanAmount", "requested_loan_amount")
    )}`,
