...
function summarizeCondition(c) {
  ...
  const valueLabel =
    valTo != null
      ? `${JSON.stringify(val)} … ${JSON.stringify(valTo)}`
      : val != null
        ? JSON.stringify(val)
        : '—';
...
/**
 * Human-readable summary of what approving / publishing a policy version does on the portal.
 */
export async function getPolicyVisibilitySummary(id) {
    ...
    snapshotHighlights: snapshotKeys.slice(0, 24).map((key) => ({
      ...
      value:
        typeof snapshot[key] === 'object'
          ? JSON.stringify(snapshot[key]).slice(0, 120)
          : String(snapshot[key]),
    })),
  };
  ...
  const publishing = {
    ...
    effects: [
      detail.bank_product_id
        ? `Bank product catalog entry will be updated with this policy snapshot (${detail.product_name || detail.bank_product_id}).`
        : 'Lender-scoped rules will go live for products under this bank (no single product row update).',
      ...
      currentActive
        ? `Current live version "${currentActive.version_label}" will be superseded (no longer active on portal).`
        : 'No other active version for this product — this will become the first live policy.',
