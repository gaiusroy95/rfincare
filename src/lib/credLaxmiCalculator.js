...
/**
 * Normalize card + optional reward_rules into engine-ready rules.
 * Cards without structured rules get a conservative 1% base cashback default.
 */
export function normalizeRewardRules(card = {}) {
  ...
  const waiver = toNum(
    fromCard?.fees?.annual_fee_waiver_spend_threshold ??
      card.annualFeeWaiverSpendThreshold ??
      card.annual_fee_waiver_spend_threshold,
    NaN,
  );

  const redemption = toNum(
    fromCard?.reward_currency?.redemption_value_inr ??
      fromCard?.redemptionValueInr ??
      fromCard?.redemption_value_inr,
    NaN,
  );

  let earnRates = Array.isArray(fromCard?.earn_rates)
    ? fromCard.earn_rates
    : Array.isArray(fromCard?.earnRates)
      ? fromCard.earnRates
      : [];
  ...
  if (usedDefaultRules) {
    // 1 point per ₹100 with ₹1 redemption ≈ 1% cashback on base spend
    earnRates = [
      {
  ...
  const welcome = Array.isArray(fromCard?.welcome_benefits)
    ? fromCard.welcome_benefits
    : Array.isArray(fromCard?.welcomeBenefits)
      ? fromCard.welcomeBenefits
      : [];
  ...
  const milestones = Array.isArray(fromCard?.milestone_benefits)
    ? fromCard.milestone_benefits
    : Array.isArray(fromCard?.milestoneBenefits)
      ? fromCard.milestoneBenefits
      : [];

  const loungeVisits = toNum(
    fromCard?.additional_benefits?.lounge_visits_per_year ??
      fromCard?.loungeVisitsPerYear ??
      (card.loungeAccess || card.lounge_access ? 2 : 0),
    0,
  );
  const loungeValue = toNum(
    fromCard?.additional_benefits?.lounge_visit_value_inr ??
      fromCard?.loungeVisitValueInr ??
      (loungeVisits > 0 ? 1500 : 0),
    0,
  );
...
function findRate(rules, categoryCode) {
  ...
  return (
    rules.earnRates.find((r) => r.categoryCode === code) ||
    rules.earnRates.find((r) => r.categoryCode === BASE_CATEGORY) ||
    null
  );
}

/**
 * Score one card's Net Annual Value under a spend profile.
 */
export function scoreCredLaxmiCard(card, spendProfile = []) {
  const rules = normalizeRewardRules(card);
  const redemption = rules.rewardCurrency.redemptionValueInr;

  const spends = (spendProfile || [])
    .map((s) => ({
  ...
  if (
    ...
    totalAnnualSpend >= waiverThreshold
  ) {
...
/**
 * Rank compared cards by highest Net Annual Value.
 */
export function rankCredLaxmiCards(cards, spendProfile) {
