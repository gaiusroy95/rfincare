...
export function getSurepassConfig(vendor = {}) {
  ...
  const timeoutMs = Number(env('SUREPASS_TIMEOUT_MS', '25000')) || 25000;
...
async function parseResponse(res) {
  ...
  try {
    ...
    return { pdfBuffer: null, json: { raw: text.slice(0, 2000) } };
  }
}
...
async function loginForToken(cfg) {
  ...
  if (!res.ok || !token) {
    const message =
      walk(json, ['message', 'error', 'msg']) || `Surepass login failed (${res.status})`;
...
export async function requestSurepassCibilPdf(demographics, vendor = {}) {
  ...
  if (!surepassConfigured(vendor)) {
    return {
      ...
      errorMessage:
        'Surepass credentials missing. Set SUREPASS_TOKEN or SUREPASS_ID_NUMBER + SUREPASS_PASSWORD in backend/.env',
    };
  }
  ...
  if (!last?.res?.ok || !successFlag) {
    return {
      ...
      errorMessage:
        String(walk(json, ['message', 'error', 'msg']) || `Surepass CIBIL failed (${last?.res?.status || 502})`),
      response: json,
...
export function saveCibilPdfBuffer(pdfBuffer, fileStem) {
