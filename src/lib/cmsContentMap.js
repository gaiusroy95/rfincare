/**
 * Normalize CMS / public content rows across Postgres alias casings.
 * Unquoted aliases become lowercase (blogurl); quoted/"snake" variants also appear.
 */
...
export function mapHomepageNewsRow(row) {
  if (!row) return null;
  return {
    id: row.id,
    title: row.title,
...
export function mapHomepageVideoRow(row) {
  if (!row) return null;
  return {
    id: row.id,
    title: row.title,
