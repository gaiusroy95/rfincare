...
/** Runtime credentials: DB overrides env when present. */
export async function resolveRazorpayCredentials() {
  ...
  const keyId =
    String(row?.key_id || '').trim() || String(process.env.RAZORPAY_KEY_ID || '').trim();
  const keySecret =
    String(row?.key_secret_encrypted || '').trim()
    || String(process.env.RAZORPAY_KEY_SECRET || '').trim();
  const webhookSecret =
    String(row?.webhook_secret_encrypted || '').trim()
...
export async function savePaymentGatewaySettings(body = {}, updatedBy) {
  const pool = getPool();
  await ensureTable(pool);
  const isEnabled = body.isEnabled !== false && body.is_enabled !== false;
  const mode = String(body.mode || 'live').toLowerCase() === 'test' ? 'test' : 'live';
  const keyId = body.keyId !== undefined ? String(body.keyId || '').trim() : undefined;
  const keySecret =
    body.keySecret !== undefined ? String(body.keySecret || '').trim() : undefined;
  const webhookSecret =
    body.webhookSecret !== undefined ? String(body.webhookSecret || '').trim() : undefined;
  ...
  await pool.execute(
     ...
     WHERE id = :id`,
    {
      ...
      mode,
      key_id: keyId === undefined ? null : keyId || null,
      set_key_secret: keySecret !== undefined && keySecret !== '' ? 1 : 0,
      key_secret: keySecret || null,
      set_webhook_secret: webhookSecret !== undefined && webhookSecret !== '' ? 1 : 0,
