...
export function normalizeReferralProgram(value) {
...
export async function ensureReferralSchema(pool = getPool()) {
...
export async function resolveReferralCode(pool, rawCode) {
...
export async function ensureReferralCodeForUser(pool, { userId, role, program }) {
  await ensureReferralSchema(pool);
...
export function buildReferralShareLinks(code, program) {
...
export async function applyReferralToLead(pool, leadId, body = {}) {
  ...
  try {
    ...
    /* engine optional until migrated */
  }
...
export async function createReferralInvite(pool, {
  referrerUserId,
  referrerRole,
  program,
  referredName,
...
export async function listReferralInvites(pool, { referrerUserId, program }) {
...
export async function countAttributedReferrals(pool, { referrerUserId, program }) {
  const [[row]] = await pool.execute(
    program
      ? `SELECT COUNT(*)::int AS c
         FROM marketing_leads
         WHERE referred_by_user_id = :user_id
           AND referral_program = :program`
      : `SELECT COUNT(*)::int AS c
         FROM marketing_leads
         WHERE referred_by_user_id = :user_id`,
...
export async function recordReferralConversion(pool, {
  referralCode,
  program = 'agent',
  referredName,
  referredEmail,
  ...
  const persisted = resolved.id
    ? resolved
    : await ensureReferralCodeForUser(pool, {
