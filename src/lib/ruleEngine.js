/**
 * Generic operator-based condition evaluation for policy rules.
 * Operators: =, !=, >, <, >=, <=, IN, BETWEEN, CONTAINS, NOT_IN
 */
...
export function resolveApplicantField(applicant, fieldKey) {
...
export function evaluateCondition(condition, applicant) {
  ...
  switch (operator) {
    ...
    case 'LTE':
      ...
      break;
    case 'BETWEEN':
      pass = actualNum != null && expectedNum != null && toNum != null
        && actualNum >= expectedNum && actualNum <= toNum;
  ...
  return {
    ...
    pass,
    reason,
    evaluated: actual !== undefined && actual !== null && actual !== '',
  };
}

/**
 * Evaluate a rule with conditions. AND within default group; OR groups pass if any condition passes.
 */
export function evaluateRule(rule, conditions, applicant) {
...
export function summarizeDecision(ruleResults, { eligibleMin = 70, conditionalMin = 50, probability = 0 } = {}) {
