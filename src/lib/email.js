...
/** Prefer Admin → OTP MSG91 email fields when server env is incomplete. */
...
async function trySendViaMsg91({
  ...
  if (!authKey || !domain || !from) {
    return {
      ...
      warningInternal:
        'MSG91 email not configured (need AUTH_KEY + EMAIL_DOMAIN + FROM). '
        + 'Set env vars or Admin → OTP settings.',
    };
  }
  ...
  if (anySent) {
    return {
      ...
      channel: 'msg91',
      attachmentCount,
      smtpFallback: smtpFallback || undefined,
      to: recipients,
