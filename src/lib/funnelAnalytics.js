...
export async function buildFunnelAnalytics(pool, { days = 30, dateFrom, dateTo } = {}) {
  ...
  for (const app of loanRows || []) {
    ...
    const createdByRole = app.lead_sourced_agent_code
      ? 'agent'
      : app.lead_assigned_to
        ? 'employee'
        : 'system';
  ...
  const productBuckets = Array.from(bucketMap.entries())
    .map(([productKey, bucket]) => ({
      productKey,
      ...bucket,
  ...
  const productMix = productBuckets
    .map((b) => ({ label: b.productLabel, count: b.leadsCreated + b.conversions }))
    .filter((i) => i.count > 0)
...
export async function buildProductConversionRows(pool, options = {}) {
