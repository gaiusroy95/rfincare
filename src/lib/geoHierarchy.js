...
export async function ensureGeoSchema(pool = getPool()) {
...
export async function listTehsils({ districtId, cityId } = {}) {
  await ensureGeoSchema();
  const pool = getPool();
...
export async function listVillages({ districtId, tehsilId } = {}) {
  await ensureGeoSchema();
  const pool = getPool();
  const clauses = ['is_active = TRUE'];
  const params = {};
...
export async function createGeoNode(level, payload) {
  await ensureGeoSchema();
  const pool = getPool();
  const id = newId();
  if (level === 'district') {
...
export async function listServiceability({ bankId } = {}) {
  await ensureGeoSchema();
  const pool = getPool();
  if (bankId) {
    const [rows] = await pool.query(
...
/** Seed a small demo hierarchy for key metros if empty. */
export async function seedDemoGeoIfEmpty() {
  await ensureGeoSchema();
  const pool = getPool();
