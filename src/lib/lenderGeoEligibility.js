/**
 * Lender geo eligibility (bank-level) — separate from FOIR/LTV product rules.
 * Hierarchy v1: State → District → Tehsil → PIN
 * Match: PIN rule first → else same-district. Most-specific wins.
 */
...
export const GEO_MESSAGES = {
  covered:
    'Your application will get approved by below lenders as probability score shown.',
  inReview:
    'Your application is in review — our agent will connect with you shortly for more information to help you in checking your eligibility.',
};
...
export function isPurchaseOrConstructionPurpose(loanTypeOrPurpose) {
  return PURCHASE_CONSTRUCTION.test(String(loanTypeOrPurpose || ''));
}

export function isSecuredLoanCategory(loanCategory) {
  return String(loanCategory || '').toLowerCase() === 'secured';
}

/**
 * Build the address PIN checks required for this application.
 * @returns {{ pins: Array<{ role: string, pincode: string|null }>, mode: string }}
 */
export function buildAddressPinChecks(input, { loanCategory } = {}) {
  const current = normalizeApplicationPincode(
    input.currentPincode ||
      input.current_pin_code ||
      input.residencePincode ||
      input.pincode ||
      input.pinCode ||
      input.pin_code,
  );
  ...
  // Unsecured: current first, permanent fallback
  return {
    mode: 'unsecured',
    pins: [
...
/**
 * Pick best matching coverage row for a PIN within one bank's active rules.
 * Priority: PIN EXCLUDE/INCLUDE/CONDITIONAL/BRANCH → district-level → none
 */
export function matchBankGeoForPin(coverageRows, { pincode, districtName = null } = {}) {
  ...
  if (distKey) {
    const districtRows = rows.filter(
      (r) =>
        (r.geo_level === 'district' || (!r.pincode && r.district_name)) &&
        r.district_name === distKey,
    );
...
/**
 * Evaluate one bank's geo for the application address set.
 */
export function evaluateBankGeoCoverage({
  bankHasGeoPolicy,
  coverageRows,
  addressChecks,
  districtName = null,
  ...
  const isReview = (r) =>
    r.outcome === 'conditional' || r.outcome === 'branch_dependent';
  const isFail = (r) =>
    r.outcome === 'excluded' ||
    r.outcome === 'not_covered' ||
    r.outcome === 'missing_pin';
  ...
  // Unsecured: current first, permanent fallback → In Review
...
/**
 * Split banks into scored (Covered / geo-skipped) vs in-review list.
 */
export function partitionBanksByGeo(bankResults) {
  const scored = [];
  const inReview = [];
...
export function buildOverallGeoMessage({ scored, inReview }) {
