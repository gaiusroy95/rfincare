...
adminHierarchyRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const pool = getPool();
      // Default: agent × L1–L4 matrix (admin Hierarchy page). ?flat=1 returns legacy rows.
...
const HIERARCHY_CSV_HEADER =
  'agent_code,agent_name,location,level1_name,level1_email,level1_mobile,level2_name,level2_email,level2_mobile,level3_name,level3_email,level3_mobile,level4_name,level4_email,level4_mobile,office_address';
