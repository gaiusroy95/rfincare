...
cmsRouter.get('/marketplace-hero', async (req, res, next) => {
  try {
    const { getAllMarketplaceHeroContent, getMarketplaceHeroContent } = await import(
      '../lib/marketplaceHeroContent.js'
    );
  ...
  try {
    ...
    if (channel === 'email') {
      ...
      return res.json({
        success: true,
        channel,
        delivered: result?.email?.delivered !== false && result?.email?.sent !== false,
        provider: result?.email?.provider || settings.emailProvider,
        warning: result?.email?.warning,
        ...(process.env.LOG_OTP === 'true' || settings.emailProvider === 'console'
          ? { devOtp: otp }
          : {}),
        result,
      });
